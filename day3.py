def build_priorities_dict():
    lower = {chr(ascii_code): idx + 1 for idx, ascii_code in enumerate(range(97, 97 + 26))}
    upper = {chr(ascii_code): idx + 27 for idx, ascii_code in enumerate(range(65, 65 + 26))}
...
def get_priorities_for_groups_of_three(input_file):
    ...
    for idx, line in enumerate(lines):
        if idx % 3 == 0:
            ...
            intersection = list(set(elf_1).intersection(set(elf_2)).intersection(set(elf_3)))
            common_item = intersection[0]
            total += priorities_dict[common_item]
        else:
            continue
